...
namespace Launcher
{
	public class MainApp : IDisposable
	{
		...
		public MainApp()
		{
			if (!Enum.TryParse(LauncherSettings.Default.HotKey, out _keys))
			{
				_keys = Keys.Oemtilde;
			}
			_modifierKeys = (ModifierKeys)0u;
			string[] array = LauncherSettings.Default.HotKeyModifier.Split(' ');
			for (int i = 0; i < array.Length; i++)
			{
				if (!Enum.TryParse(array[i], out ModifierKeys result))
				{
					_modifierKeys = ModifierKeys.Win;
					break;
				}
				_modifierKeys |= result;
			}
